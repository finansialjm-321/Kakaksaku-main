...
export default function KakasakuPackages() {
  ...
  // STATE BARU: Untuk simpan nominal Jayawijaya
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      if (data) {
        // REVISI 1: Sortir ulang biar Jayawijaya selalu di paling bawah
        const sortedPackages = data.sort((a, b) => {
          if (a.name === 'Jayawijaya') return 1;
          if (b.name === 'Jayawijaya') return -1;
  ...
  // HANDLER: Pastikan cuma bisa masukin Angka
  ...
  const handleConfirmPackage = async () => {
    if (!selectedPkg || !isAgreed) return;

    const finalAmount = selectedPkg.name === 'Jayawijaya' ? Number(customAmount) : selectedPkg.amount;

    // Double validation di fungsi submit buat jaga-jaga
    if (selectedPkg.name === 'Jayawijaya') {
      if (!customAmount || finalAmount <= 150000) {
        setErrorMessage("Nominal paket Jayawijaya harus lebih dari Rp 150.000!");
        return;
      }
    }
  ...
  const handleSelectPackage = (pkg: any) => {
    setSelectedPkg(pkg);
    setIsAgreed(false); 
    // Jangan reset input custom kalau yang dipilih itu Jayawijaya
    if (pkg.name !== 'Jayawijaya') {
      setCustomAmount(''); 
    }
    setTimeout(() => {
  ...
  // Cek apakah input Jayawijaya valid (buat disable tombol)
  const isJayawijayaInvalid = selectedPkg?.name === 'Jayawijaya' && Number(customAmount) <= 150000;
            ...
            return (
              ...
              >
                <CardContent className="p-8 flex flex-col items-center text-center">
                  ...
                  {/* REVISI 2: INPUT JAYAWIJAYA MASUK KE DALAM CARD */}
                  <div className="mb-4 w-full">
                    {pkg.name === 'Jayawijaya' ? (
                      <div className="mt-2 mb-4">
                        <span className={`text-xl font-black transition-colors block mb-3 ${isSelected ? 'text-orange-600' : 'text-[#1A1A1A]'}`}>
                          Bebas Tentukan
                        </span>
                        <div className="relative w-full">
                          ...
                          <input 
                            type="text"
                            inputMode="numeric"
                            placeholder="Min. 150001"
                            className="w-full pl-10 pr-4 py-2 border-2 border-orange-300 rounded-xl text-right font-black text-orange-600 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition-all bg-white"
                            value={customAmount}
                            onClick={(e) => {
                              e.stopPropagation(); // Biar pas klik input, card-nya tetep kepilih tapi gak dobel render
                              handleSelectPackage(pkg);
                            }}
                            onChange={(e) => {
                              handleCustomAmountChange(e);
                              if (!isSelected) handleSelectPackage(pkg);
                            }}
                          />
                        </div>
                        <span className="text-[11px] font-bold text-muted-foreground block mt-2">&gt; Rp 150.000 / bulan</span>
                      </div>
                    ) : (
                      <>
            ...
            <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-gray-100">
              ...
              <div className="bg-gray-50 rounded-3xl p-6 mb-6 flex flex-col sm:flex-row justify-between items-center text-center sm:text-left border border-gray-100 gap-4">
                ...
                <div className="sm:text-right w-full sm:w-auto border-t sm:border-t-0 pt-4 sm:pt-0 border-gray-200">
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Komitmen Bulanan</p>
                  
                  {/* Total harga di-update real-time sesuai form */}
                  <p className="text-2xl font-black text-orange-500">
                    Rp {selectedPkg.name === 'Jayawijaya' 
                      ? (customAmount ? Number(customAmount).toLocaleString('id-ID') : '0') 
                      : selectedPkg.amount.toLocaleString('id-ID')}
                  </p>
                </div>
              </div>
              ...
              {/* REVISI 3: VALIDASI TOMBOL DISABLE KALAU JAYAWIJAYA < 150k */}
              <Button 
                size="lg" 
                className={`w-full mt-8 py-7 text-lg font-black rounded-2xl shadow-xl transition-all duration-300 ${
                  (!isAgreed || isJayawijayaInvalid) ? 'bg-gray-300 text-gray-500 cursor-not-allowed shadow-none hover:bg-gray-300' : 'bg-orange-500 hover:bg-orange-600 text-white shadow-orange-500/30 hover:-translate-y-1'
                }`} 
                onClick={handleConfirmPackage} 
                disabled={loading || !isAgreed || isJayawijayaInvalid}
              >
                {loading 
                  ? 'Menyimpan...' 
                  : isJayawijayaInvalid 
