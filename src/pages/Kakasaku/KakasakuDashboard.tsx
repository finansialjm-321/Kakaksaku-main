...
export default function KakasakuDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // 4. Ambil Data Program Donasi (Logika Realtime seperti donasi.tsx)
    ...
    if (programsRes.data) {
      const enrichedPrograms = programsRes.data.map((p) => ({
        ...p,
  ...
  useEffect(() => {
    ...
    // SETUP REAL-TIME (Sinkronisasi tabel payments, donations, dan programs)
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "donation_programs" }, () => {
        fetchData();
      })
      .subscribe();
